...
# ---------------------------------------------------------------------------
# Trap detection — match resident blind spots to case miss reasons
# ---------------------------------------------------------------------------
# Each resident has specific trap rules: conditions under which their
# personal blind spots would cause them to misread a case.
# These are hand-authored per archetype for precision.
...
def _score_trap(resident: Resident, case: GeneratedCase) -> tuple[float, str]:
    """
...
class Shift:
    ...
    def __init__(
        ...
        # --- Trap detection ---
        # Score every bay for trap potential, flag the best one
        self._detect_traps()

    def _detect_traps(self):
        """
        ...
        for bay_id, bay in self.bays.items():
            score, detail = _score_trap(bay.resident, bay.case)
        ...
        for i in range(len(bay_ids)):
            for j in range(i + 1, len(bay_ids)):
                bay_a = self.bays[bay_ids[i]]
                bay_b = self.bays[bay_ids[j]]
                # Try resident A on case B
                score_ab, detail_ab = _score_trap(bay_a.resident, bay_b.case)
                ...
                score_ba, detail_ba = _score_trap(bay_b.resident, bay_a.case)
        ...
        if best_swap and best_swap_score > best_score:
            ...
            id_a, id_b = best_swap
            bay_a = self.bays[id_a]
            bay_b = self.bays[id_b]
            ...
            for bay_id in [id_a, id_b]:
                score, detail = _score_trap(
                    self.bays[bay_id].resident, self.bays[bay_id].case
                )
                if score >= 0.3:
                    self.bays[bay_id].is_trap = True
                    self.bays[bay_id].trap_detail = detail
                    return
    ...
    def setup(self):
        ...
        for bay in self.bays.values():
            ...
            # Pass trap context so the LLM leans into blind spots
            trap_ctx = bay.trap_detail if bay.is_trap else ""
            assessment = bay.resident_ai.proactive(bay.case, trap_context=trap_ctx)
